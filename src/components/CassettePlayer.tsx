...
import { getMusicKitInstance } from '../services/appleMusic'
...
export function CassettePlayer() {
  ...
  useEffect(() => {
    ...
    const onStateChange = () => {
      ...
      else if (state === states.stopped || state === states.completed) setPlaybackState('stopped')
      else if (state === states.loading || state === states.waiting) setPlaybackState('loading')
    }

    const onNowPlayingChange = () => {
      const idx = music.queue.position
  ...
  async function handlePlay() {
    try {
      await getMusicKitInstance().play()
    } catch (e) {
      console.error(e)
    }
  }
