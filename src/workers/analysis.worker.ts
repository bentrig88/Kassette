...
import type { RhythmMethod } from '../services/audioAnalysis'
...
self.onmessage = async (e: MessageEvent<AnalyzeRequest>) => {
  const { reqId, id, samples, sampleRate, method } = e.data
  try {
    const features = await analyzePCM(id, samples, sampleRate, method)
    post({ reqId, features })
  } catch (err) {
